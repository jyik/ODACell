...
class DobotApiDashboard(DobotApi):
    ...
    def DOExecute(self,offset1,offset2):
        ...
        index: Digitaloutput index (Value range 1~24)
        status: Status of digital signal output port(0:Low level, 1:High level)
        """
...
class DobotApiMove(DobotApi):
    ...
    def MovJ(self, x, y, z, rx, *dynParams):
        """
        ...
        ry: Position of Ry axis in Cartesian coordinate system (removed)
        rz: Position of Rz axis in Cartesian coordinate system (removed)
        """
        string = "MovJ({:f},{:f},{:f},{:f},0.0,0.0".format(
            x, y, z, rx)
        for params in dynParams:
            string = string + "," + str(params)
        string = string + ")"
    ...
    def MovL(self, x, y, z, rx, *dynParams):
        ...
        rz: Position of Rz axis in Cartesian coordinate system (removed)
        """
        string = "MovL({:f},{:f},{:f},{:f},0.0,0.0".format(
            x, y, z, rx)
        for params in dynParams:
            string = string + "," + str(params)
        string = string + ")"
    ...
    def JointMovJ(self, j1, j2, j3, j4, *dynParams):
        ...
        """
        string = "JointMovJ({:f},{:f},{:f},{:f},0.0,0.0".format(
            j1, j2, j3, j4)
        for params in dynParams:
            string = string + "," + str(params)
        string = string + ")"
    ...
    def RelMovJ(self, offset1, offset2, offset3, offset4, *dynParams):
        """
        Offset motion interface (point-to-point motion mode)
        j1~j4:Point position values on each joint
        """
        string = "RelMovJ({:f},{:f},{:f},{:f},0.0,0.0".format(
            offset1, offset2, offset3, offset4)
        for params in dynParams:
            string = string + "," + str(params)
        string = string + ")"
