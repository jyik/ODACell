...
def construct_script(args, cluster_oe_dir):
    ...
    if "hvarfnec" in os.getcwd():
        script.append(f"#SBATCH --{args.partition}")
        script.append(f"#SBATCH --mem {args.memory}G")
        script.append(f'#SBATCH --array 0-{num_tasks-1}%{args.max_tasks}')
        script.append(f'#SBATCH -c {args.cores}')

    elif 'x_carhv' in os.getcwd():
        script.append(f"#SBATCH -A {args.partition}")
        script.append(f'#SBATCH -n {args.cores, 2}')
        script.append(f'#SBATCH --array 0-{num_tasks-1}')
        script.append(f'#SBATCH --mem-per-cpu {args.memory}000')

    else:
        raise ValueError("Not a valid user, not submitting.")
...
if __name__ == "__main__":
    ...
    if "hvarfnec" in os.getcwd():
        parser.add_argument("--partition", default="bosch_cpu-cascadelake")
        parser.add_argument("--cores", default=6)
        parser.add_argument("--memory", default=30, type=int) # total
    else:
        parser.add_argument("--partition", default="naiss2023-22-1024")
        #parser.add_argument("--project", default="naiss2023-5-189") #Ours
        parser.add_argument("--cores", default=1)
        parser.add_argument("--memory", default=6, type=int) # per cpu
