...
def get_designs(
    ...
    for _ in range(max_num_designs):
        ...
        batch_array.append((*trial, df.loc[df['trial_index'] == trial[1]]['generation_method'].values[0]))
...
def add_trial(param_dic, client = None, client_path = None, save = True):
    if client is None:
        client = _get_client(client_path + AX_NAME)
    trial = client.attach_trial(param_dic)
    if save:
